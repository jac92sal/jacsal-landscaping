...
| `arcgis-js-sdk` | Need an interactive browser map or 3D scene: web components (`arcgis-map`, `arcgis-scene`), graphics, feature layers, BuildingSceneLayer + Slice for Revit/BIM models, ElevationLayer / ground / `queryElevation` / elevation profile, React/Vite setup. |
| `arcgis-api-keys` | Need a long-lived ArcGIS API key, key rotation, privilege identifiers, or referrer setup: REST JS `createApiKey` and the raw portal REST sequence. |
